...
CREATE TABLE flags_1 (
  id SERIAL PRIMARY KEY,
  country_id INT UNIQUE REFERENCES countries_1,
  created_at TIMESTAMP DEFAULT current_timestamp,
  -- FOREIGN KEY (country_id) REFERENCES countries_1 (id)
);

ALTER TABLE countries_1
ADD COLUMN flag_id INT NOT NULL UNIQUE REFERENCES flags_1 (id);
...
CREATE TABLE flags_2 (
  id SERIAL PRIMARY KEY,
  country_id INT NOT NULL UNIQUE REFERENCES countries_2 (id) ON DELETE CASCADE ON UPDATE CASCADE DEFERRABLE INITIALLY DEFERRED,
  created_at TIMESTAMP DEFAULT current_timestamp
);

ALTER TABLE countries_2
ADD COLUMN flag_id INT NOT NULL UNIQUE REFERENCES flags_2 (id) ON DELETE CASCADE ON UPDATE CASCADE  DEFERRABLE INITIALLY DEFERRED;
